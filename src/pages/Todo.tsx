...
function SortableItem({ todo, onToggle, onDelete, onEdit }: {
  ...
  return (
    <div ref={setNodeRef} style={style}
      ...
      <span
        className={`flex-1 text-sm ${todo.is_completed ? 'line-through text-gray-400' : 'text-gray-700 cursor-pointer hover:text-indigo-600'}`}
        onClick={() => !todo.is_completed && onEdit(todo)}>
        {todo.title}
      </span>
      <button onClick={() => onDelete(todo.id)} className="text-gray-300 hover:text-red-400 text-lg">×</button>
    </div>
  )
}
...
export default function Todo({ userId }: { userId: string }) {
  ...
  const addTodo = async () => {
    ...
    await supabase.from('todos').insert({
      title, user_id: userId, is_completed: false, position: todos.length,
    })
    setTitle('')
    await fetchTodos()
    setLoading(false)
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex gap-2 mb-6">
        <input type="text" placeholder="새 할 일 입력..."
